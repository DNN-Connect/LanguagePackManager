...
namespace Connect.LanguagePackManager.Core.Services.Packages
{
  ...
  {
    private static readonly ILog Logger = LoggerSource.Instance.GetLogger(typeof(Manifest));
    ...
    {
      Logger.Info($"Loading manifest {unzipResult.ManifestFile}");
      ...
      Logger.Info($"Manifest Version {this.ManifestVersion}");
